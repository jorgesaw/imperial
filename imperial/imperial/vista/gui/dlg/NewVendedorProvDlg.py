...
from imperial.vista.gui.dlg import ui_dlgAddVendedorProv
...
class NewVendedorProvDlg(_qg.QDialog,
                   ui_dlgAddVendedorProv.Ui_DlgAddVendedorProv):
    u"""Diálogo para agregar los datos de un vendedor."""

    def __init__(self, modelo=None, parent=None):
        super(NewVendedorProvDlg, self).__init__(parent)
    ...
    def buscarProveedor(self):
        ...
        busqueda = FactoriaVentanasSearch.crearVentanaGui(factoria.SEARCH_VENDEDOR_PROV, self, 
              {'carga_previa': False})
        proveedor = busqueda.datoBuscado()
        
        if proveedor:
            self.proveedor = proveedor
            self.vendedorLineEdit.setText(self.proveedor.nombre)
    ...
    def data(self):
        lstDatosProv = [unicode(self.nombreLineEdit.text()), unicode(self.emailLineEdit.text()),  
                ...
                datetime.date.today(), self.proveedor]
