...
class ModeloDatosTablaEgreso(object):
    u""""""
    ...
    lstTitHeader = ['Nombre egreso', u'Editar', 'Eliminar']
    def __init__(self):
        pass
    ...
    def data(self, index, dato, role=_qc.Qt.DisplayRole):
        ...
        if role == _qc.Qt.DisplayRole:
            if column == ModeloDatosTablaEgreso.NAME:
                return _qc.QVariant(dato.nombre)
            elif column == ModeloDatosTablaEgreso.EDITAR:
                return _qc.QVariant('Editar')
            elif column == ModeloDatosTablaEgreso.ELIMINAR:
                return _qc.QVariant('Eliminar')
            
        elif role == _qc.Qt.DecorationRole:
            if column == ModeloDatosTablaEgreso.EDITAR:
                return _qg.QIcon(variables.ICON_PATH_EDIT)
            if column == ModeloDatosTablaEgreso.ELIMINAR:
    ...
    def headerData(self, section, orientation, role=_qc.Qt.DisplayRole):
        ...
        if role != _qc.Qt.DisplayRole:
            return _qc.QVariant()
        if orientation == _qc.Qt.Horizontal:
            if section in ModeloDatosTablaEgreso.lstHeader:
                return _qc.QVariant(ModeloDatosTablaEgreso.lstTitHeader[section])
