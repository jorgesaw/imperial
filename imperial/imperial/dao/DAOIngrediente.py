...
class DAOIngrediente(DAOAlchemy):

    def __init__(self, cerrarSesion=True):
        super(DAOIngrediente, self).__init__(cerrarSesion)
    ...
    def getListaDatos(self, mapParam):
        ...
        objects = []
        try:
            objects = self.session.query(Ingrediente).filter(Ingrediente.nombre.like(
    ...
    def datosByCol(self, mapParam):
        objects = []
        try:
            objects = self.session.query(Ingrediente).filter(\
                           Ingrediente.nombre == mapParam['COL']).\
                          order_by(asc(collate(Ingrediente.nombre, 'NOCASE'))).all()
        except sqlalchemy.exc.DBAPIError, e:
            if self.session is not None:
                self.session.rollback()
            print("Error!", e)
